 ...
 */
 function shallowCopy(src) {
  ...
  const constructor = src.constructor; // src的构造函数
  if (/^(Date|Map|Set|Array|RegExp)$/i.test(constructor.name)) {
    // 内部类不考虑，直接返回
    return src;
  }
  const cloneSrc = new constructor(); // 使用相应的构造函数创建和原对象同类型的实例cloneSrc
  cloneSrc.__proto__ = src.__proto__; // 将cloneSrc的隐式原型指向src的原型，实现原型链继承
  for (let prop in src) {
    // 遍历拷贝src自身的属性
    if (src.hasOwnProperty(prop)) {
      cloneSrc[prop] = src[prop];
    }
  }
  return cloneSrc;
}
...
function createWeightedRandom(input = []) {
  ...
  const total = input.reduce((prev, next) => prev + next, 0); // 权重总和
  // 根据input计算出来的前缀和数组，pre[i] = input[0] + ... + input[i-1] + input[i]
  const preArr = [input[0]];
  for (let i = 1; i < input.length; i++) {
    preArr[i] = preArr[i-1] + input[i];
  }

  return () => {
    const x = Math.ceil(Math.random() * total); // 生成total范围内的随机数
    // 使用二分法查找x对应的下标，preArr[i] - input[i] + 1 <= x <= preArr[i]
    let left = 0;
    let right = preArr.length - 1;
    while (left < right) {
      const mid = Math.floor((left + right) / 2); // 中间位置
      if (preArr[mid] === x) { // 直接命中当前随机数，返回mid
        return mid;
      }      
      if (preArr[mid] < x) {
        left = mid + 1;
    ...
    // while循环结束，left = right, 此时left就是最后的下标
    return left;
  }
}
/**
...
function curry(fn, ...existingArgs) {
  ...
  const that = this;
  const fnArgLen = fn.length; // 传入函数的形参个数 

  // 传入参数个数 >= 函数的形参个数，直接调用fn
  if (existingArgs.length >= fnArgLen) {
    return fn.apply(that, existingArgs);
  }
  // 传入参数个数 < 函数的形参个数, 则返回函数，继续递归调用收集参数
  return function () {
    const nextArgs = [...arguments];
    return curry.call(that, fn, ...existingArgs, ...nextArgs);
  }
}
/**
 * Q4: 异步并发控制器
...
function createAsyncWorker(capacity) {
  const undoQueue = []; // 存储所有的任务
  let count = 0; // 记录当前正在执行的异步函数个数

  return async function executor(task) {
    return new Promise(async (resolve, reject) => {
      if (count >= capacity) {
        // 当前执行的异步函数个数超过容量时，暂停往下走，使用Promise resolve放入undoQueue中实现阻塞，等待某个task执行结束后再恢复
        await new Promise(resolve => undoQueue.push(resolve));
      }

       // 从未完成的任务队列里面弹出之前存储的resolve函数并执行，恢复上面的阻塞流程
      const next = () => {
        if (undoQueue.length) {
          const head = undoQueue.shift();
          head();
        }
      }

      // 未超出容量，立即执行，并记录执行函数个数
      try {
        count++; // 执行完毕，执行个数+1
        const result = await task(); // 执行task
        count--; // 执行完毕，执行个数-1
        next();
